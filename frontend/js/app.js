document.addEventListener("alpine:init", () => {
  Alpine.data("translationApp", () => ({
    ...
    inputData: {
      ...
      config: {
        ...
        target_language_accent:
          localStorage.getItem("target_accent") || "professional",
        translation_mode:
          localStorage.getItem("translation_mode") || "deep_mode",
        chunking_algorithm:
          localStorage.getItem("chunking_algorithm") || "smart",
    ...
    availableModels: [],
    status: {
      // Holds the latest state received from the stream
      current_step: null,
      progress_percent: 0,
      logs: [],
    ...
    canStartTranslation() {
      return (
        this.inputData.original_content.trim() !== "" &&
        ...
        ((this.inputData.config.provider && this.inputData.config.model) || // Explicitly set
          (this.defaultLLMConfig &&
            this.defaultLLMConfig.provider &&
            this.defaultLLMConfig.model)) // Or default exists
      );
    },
    ...
    init() {
      ...
      this.$watch("inputData.config.source_lang", (val) =>
        localStorage.setItem("source_lang", val)
      );
      this.$watch("inputData.config.target_lang", (val) =>
        localStorage.setItem("target_lang", val)
      );
      this.$watch("inputData.config.provider", (val) => {
        localStorage.setItem("provider", val);
        this.inputData.config.model = "";
        localStorage.removeItem("model");
      }); // Reset model on provider change
      this.$watch("inputData.config.model", (val) =>
        localStorage.setItem("model", val)
      );
      this.$watch("inputData.config.target_language_accent", (val) =>
        localStorage.setItem("target_accent", val)
      );
      this.$watch("inputData.config.translation_mode", (val) =>
        localStorage.setItem("translation_mode", val)
      );
      this.$watch("inputData.config.chunking_algorithm", (val) =>
        localStorage.setItem("chunking_algorithm", val)
      );
      this.$watch("inputData.config.symbol_separators", (val) =>
        localStorage.setItem("symbol_separators", val)
      );
      ...
      this.$watch("defaultLLMConfig", (config) => {
        if (config) {
          ...
          this.inputData.config.target_language_accent =
            config.target_language_accent;
          ...
          if (config.chunking_algorithm) {
            this.inputData.config.chunking_algorithm =
    ...
    updateModels() {
      ...
      const modelsForProvider =
        this.availableProviders[this.inputData.config.provider] || [];
      console.log("Raw models for provider:", modelsForProvider);

      if (this.modelFilter) {
        this.availableModels = modelsForProvider.filter((m) =>
          m.toLowerCase().includes(this.modelFilter.toLowerCase())
        );
      } else {
    ...
    calculateDuration() {
      ...
      const durationSeconds = Math.round(
        (endTime - this.translationStartTime) / 1000
      );
    ...
    async startTranslation() {
      if (!this.canStartTranslation()) {
        ...
        alert(
          "Please fill in all required fields (API URL, Languages, Provider, Model) and provide input text."
        );
        return;
      }
      ...
      if (
        (!configToSend.provider || !configToSend.model) &&
        this.defaultLLMConfig &&
        this.defaultLLMConfig.provider &&
        this.defaultLLMConfig.model
      ) {
        console.log(
          "Using default LLM config for submission:",
          this.defaultLLMConfig
        );
        ...
        console.log(
          "Using explicitly set config for submission:",
          configToSend
        );
      }
    ...
    connectToJobStream(jobId) {
      ...
      this.eventSource.onmessage = (event) => {
        ...
        try {
          ...
          this.status.current_step =
            eventData.current_step || eventData.status || "Processing";
          ...
          if (eventData.recent_logs) {
            this.logs = eventData.recent_logs.map(
              (log) => `${log.created_at} [${log.level}] ${log.message}`
            );
          }
    ...
    async calculateJobStats() {
      ...
      try {
        ...
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/jobs/statistics`
        );
    ...
    async fetchJobDetails(jobId) {
      try {
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/jobs/${jobId}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
    ...
    downloadTranslation(jobId) {
      window.open(
        `${this.apiUrl.replace(/\/$/, "")}/jobs/${jobId}/download`,
        "_blank"
      );
    },
    ...
    async fetchLLMConfigs() {
      try {
        ...
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/llm-configs`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
    ...
    async fetchDefaultLLMConfig() {
      try {
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/llm-configs/default`
        );
        if (response.ok) {
          this.defaultLLMConfig = await response.json();
          console.log(
            "Default LLM configuration fetched:",
            this.defaultLLMConfig
          );
    ...
    updateConfigModels() {
      const modelsForProvider =
        this.availableProviders[this.newLLMConfig.provider] || [];
    ...
    canSaveLLMConfig() {
      ...
      if (
        this.newLLMConfig.chunking_algorithm === "symbol" &&
        !this.isValidSymbolSeparators()
      ) {
        return false;
      }
    ...
    async setDefaultLLMConfig(configId) {
      try {
        ...
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/llm-configs/${configId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
    ...
    async fetchEnvVariables() {
      try {
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/env-variables`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
    ...
    editLLMConfig(config) {
      ...
      this.newLLMConfig = {
        api_url:
          config.api_url ||
          `${window.location.protocol}//${window.location.hostname}:8051`,
      ...
      document
        .querySelector(
          ".p-4.border.border-\\[var\\(--border-color\\)\\].rounded-md"
        )
        .scrollIntoView({
          behavior: "smooth",
          block: "start",
    ...
    async deleteEnvVariable(key) {
      ...
      try {
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/env-variables/${key}`,
          {
            method: "DELETE",
          }
        );
    ...
    updateGlossaryJsonString() {
      try {
        this.currentGlossary.glossary_json_string = JSON.stringify(
          this.currentGlossary.terms || [],
          null,
          2
        );
    ...
    parseGlossaryJsonString(jsonString) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          this.currentGlossary.terms = parsed
            .map((term) => {
              if (
                term &&
                typeof term.sourceTerm === "string" &&
                typeof term.proposedTranslations === "object" &&
                term.proposedTranslations !== null
              ) {
    ...
    addGlossaryTerm() {
      ...
      if (!Array.isArray(this.currentGlossary.terms)) {
        console.error(
          "Error: currentGlossary.terms is not an array! Initializing.",
          this.currentGlossary
        );
        this.glossaryError =
          "Internal error: Glossary data structure invalid. Initializing.";
        this.currentGlossary.terms = []; // Initialize as fallback
      }

      // Avoid adding duplicates (simple check based on sourceTerm)
      if (
        this.currentGlossary.terms.some(
          (term) => term.sourceTerm === newEntry.sourceTerm
        )
      ) {
      ...
      if (Array.isArray(this.currentGlossary.terms)) {
        ...
        console.error(
          "Error: Failed to push to currentGlossary.terms as it's still not an array."
        );
    ...
    async saveUserGlossary() {
      ...
      if (
        !Array.isArray(this.currentGlossary.terms) ||
        this.currentGlossary.terms.length === 0
      ) {
      ...
      try {
        ...
        alert(
          `Glossary '${glossaryPayload.name}' ${isUpdate ? "updated" : "saved"} successfully!`
        );
        this.resetCurrentGlossary();
        await this.loadUserGlossaries(); // Refresh the list
      } catch (error) {
        console.error(
          `Error ${isUpdate ? "updating" : "saving"} glossary:`,
          error
        );
    ...
    async editUserGlossary(glossaryId) {
      ...
      try {
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/glossaries/${glossaryId}`
        );
        if (!response.ok) {
        ...
        this.currentGlossary = {
          ...
          glossary_json_string: JSON.stringify(
            glossaryData.glossary_data || [],
            null,
            2
          ),
        };
        ...
        document
          .querySelector('.card h3[x-text^="Edit Glossary"]')
          .scrollIntoView({ behavior: "smooth" });
      } catch (error) {
        console.error(
          `Error fetching glossary ${glossaryId} for editing:`,
          error
        );
    ...
    async deleteUserGlossary(glossaryId, glossaryName) {
      if (
        !confirm(
          `Are you sure you want to delete the glossary "${glossaryName}"? This cannot be undone.`
        )
      )
        return;
      this.glossaryError = "";
    ...
    async setDefaultUserGlossary(glossaryId) {
      ...
      try {
        const response = await fetch(
          `${this.apiUrl.replace(/\/$/, "")}/glossaries/${glossaryId}/default`,
          { method: "POST" }
        );
        const result = await response.json();
        if (!response.ok) {
        ...
        console.error(
          `Error setting glossary ${glossaryId} as default:`,
          error
        );
